...
export default class LinkWarper extends Component {
  ...
  componentWillReceiveProps(props) {
    const { currentState: { link, selectionText } } = props
    const linkUrl = (link && link.target) || ''
    const linkTitle = (link && link.title) || selectionText
    this.setState({
      linkUrl,
  ...
  render() {
    const { isShow, insertLink, removeLink, closeModal } = this.props
    const { linkTitle, linkUrl } = this.state
    return (
      <div className="link-warper">
        <AddLink onClick={insertLink} />
        <RemoveLink onClick={removeLink} />
        {isShow ? (
          <div className="link-warper-container">
            ...
            <div className="link-warper-btns">
              <button disabled={!linkUrl || !linkTitle} onClick={this.addLink}>
                添加
              </button>
